...
public class LevelUtils {
    ...
    public static int getExpRequiredForLevel(int level) {
        return (int) ((Math.pow(6F * ModConfig.get().statScalings.NORMAL_STAT_SCALING.getMultiFor(level), 2F)) * 2);
    }

    public static int getBaseExpMobReward(int level) {
        return (int) ((4 + (Math.pow(2F * ModConfig.get().statScalings.NORMAL_STAT_SCALING.getMultiFor(level), 1.3F))) * 2);
    }
